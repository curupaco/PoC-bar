...
A3	Contraste insuficiente em textos informativos. Labels em text-[8px] e text-[9px] com text-slate-400 em background claro podem não atingir 4.5:1 de contraste mínimo.	Múltiplos arquivos: Dashboard.tsx, Reports.tsx, Sidebar.tsx, POS.tsx	MÉDIA	Aumentar peso da fonte (font-bold em vez de font-black para textos pequenos) ou aumentar tamanho mínimo para 10px em textos informativos. Usar text-slate-500 no mínimo.
...
#	Problema	Local	Complex.	Correção Proposta
B1	Toast muito curto (4s em App, 2.5s no POS). Em conexões lentas ou para usuários com deficiência cognitiva, o tempo é insuficiente para ler a mensagem.	App.tsx (linha 41), POS.tsx (linha 104), ProductList.tsx (linha 68)	BAIXA	Aumentar duração para mínimo 6-8 segundos. Adicionar botão para fechar manualmente o toast.
...
B5	Feedback de sucesso inconsistente. Algumas operações mostram toast verde (ProductList), outras mostram toast vermelho (App.tsx usa toast tipo 'info' para erros apenas).	App.tsx (linhas 38-42), múltiplos componentes	BAIXA	Padronizar: erros = toast vermelho com ícone ⚠️, sucesso = toast verde, info = toast cinza/azul.
...
B7	Confirmação de exclusão usa confirm() nativa. UserManagement usa window.confirm() que é bloqueante e não estilizável, além de ser má experiência mobile.	UserManagement.tsx (linha 301)	BAIXA	Substituir por modal de confirmação customizado já existente no projeto (ConfirmationModal).
---
...
C2	Nome do cliente obrigatório para pendura não indicado previamente. Usuário só descobre ao tentar finalizar se precisa do nome.	POSPaymentPanel.tsx (linhas 125-127)	BAIXA	Quando método = Pendura selecionado, mostrar campo nome imediatamente com placeholder "Nome do cliente (obrigatório)".
...
C8	Logout sem proteção contra clicks acidentais. Botão "Sair" na sidebar executa logout diretamente (App.tsx linha 96-109 mostra modal, mas em alguns fluxos pode não).	Sidebar.tsx (linha 269), App.tsx	BAIXA	Garantir que sempre abra modal de confirmação antes de logout.
...
#	Problema	Local	Complex.	Correção Proposta
D1	Validação de produto permite preços zero. Produto com preço 0 pode ser salvo (validation apenas para < 0).	ProductList.tsx (linha 88)	BAIXA	Adicionar validação: if (numericPrice === 0) { setError("Preço deve ser maior que zero."); return; }
...
D3	Nome de produto duplicado não é validado. Pode criar dois produtos com mesmo nome (ex: "HEINEKEN 600ML" e "Heineken 600ml").	ProductList.tsx (linhas 75-127)	BAIXA	Adicionar verificação: if (products.some(p => p.name.toUpperCase() === finalName)) { setError("Produto já existe."); return; }
D4	Categorias não normalizadas na criação. Usuário pode criar "BEBIDAS", "Bebidas ", "bebidas" como categorias separadas.	ProductList.tsx (linha 79), ProductList.tsx (linhas 118-124)	BAIXA	Normalizar categoria para uppercase + trim antes de salvar e antes de verificar existência.
...
D6	Campo de busca em ProductList não filtra em tempo real - usuário precisa pressionar Enter ou esperar debounce.	ProductList.tsx (linhas 51, 230)	BAIXA	Adicionar debounce de 300ms ou filtrar em tempo real (react state é rápido o suficiente para lista < 1000 itens).
---
...
E2	Tabela de histórico de vendas não é scrollável horizontalmente em mobile.overflow-x não está implementado na tabela.	Inventory.tsx (linhas 311-352)	BAIXA	Adicionar overflow-x-auto ao container da tabela.
...
E5	Grid de produtos no POS não é responsivo para telas muito pequenas (320px). grid-cols-2 mínimo pode quebrar em celulares antigos.	POS.tsx (linha 364)	BAIXA	Usar grid-cols-1 sm:grid-cols-2 para garantir 1 coluna em telas muito pequenas.
E6	Navegação de abas em Reports/ProductList transborda em telas pequenas.Botões com flex-1 podem ficar muito comprimidos.	Reports.tsx (linhas 356-367), ProductList.tsx (linhas 217-224)	BAIXA	Usar min-w-[80px] para garantir largura mínima legível ou overflow-x-auto com scroll horizontal.
...
F2	Tamanhos de fonte inconsistentes para mesmo nível hierárquico. Labels variam entre text-[8px] a text-[12px] no mesmo componente.	Todo o projeto	ALTA	Definir scale tipográfico consistente: text-xs (12px), text-[10px] (10px), text-[9px] (9px), text-[8px] (8px). Usar apenas esses sizes.
...
#	Problema	Local	Complex.	Correção Proposta
G1	Usuário admin não pode ser editado. Ao tentar editar admin, campos aparecem preenchidos mas não há feedback claro.	UserManagement.tsx (linha 300)	BAIXA	Mostrar badge "Conta de sistema" e desabilitar edição de admin com tooltip "Contas admin devem ser editadas no banco de dados".
G2	Mensagem de "Turno Fechado" não indica quando foi fechado. Usuário não sabe há quanto tempo.	ShiftControl.tsx (linha 192)	BAIXA	Adicionar timestamp: "Aberto às HH:mm" + " (há X minutos)".
...
G6	Quantidade mínima em pagamentos não clara. Usuário não sabe se pode pagar R$ 0,01 ou se há mínimo.	POSPaymentPanel.tsx (linha 86)	BAIXA	Mostrar "Mínimo: R$ 0,05" como hint abaixo do campo de valor.
...
G8	Pesquisa em Products não mostra "nenhum resultado". Tela fica vazia sem feedback.	ProductList.tsx (filtragem apenas)	BAIXA	Adicionar estado: if (filteredProducts.length === 0 && searchTerm) showFeedback("Nenhum produto encontrado").
...
G12	Indicador de giro em vendas não indica o que é. "OCIOSA" e "TRAVADA" são confusos sem legenda.	POS.tsx (linhas 386-390)	BAIXA	Ao passar mouse/toque, mostrar tooltip: "Mesa sem atividade há X minutos".
---
RESUMO EXECUTIVO
