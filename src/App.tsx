...
export const App: React.FC = () => {
  ...
  if (!currentUser) return <Login onLogin={(u, p) => handleLogin(u, p, store.users)} isLoading={store.dbStatus === 'loading' && store.users.length === 0} error={loginError} onResetAdminPassword={store.handleResetAdminPassword} />;
  ...
  const renderActiveView = () => {
    ...
    switch (activeView) {
      ...
      case 'assistant': return <OwnerAssistant products={store.products} sales={store.sales} stockTransactions={store.stockTransactions} auditLogs={store.auditLogs} shifts={store.shifts} users={store.users} handleUpdateProducts={store.handleUpdateProducts} activeUnitId={store.validatedActiveUnitId} openTabs={store.openTabs} subscriptionPlans={store.subscriptionPlans} subscribers={store.subscribers} subscriptionLogs={store.subscriptionLogs} onSavePlan={store.handleSaveSubscriptionPlan} onDeletePlan={store.handleDeleteSubscriptionPlan} onSaveSubscriber={store.handleSaveSubscriber} onDeleteSubscriber={store.handleDeleteSubscriber} onRenewSubscriber={store.handleSimulateSubscriptionRenewal} />;
      case 'inventory': return <Inventory products={store.products} stockTransactions={store.stockTransactions} onUpdateStock={store.handleUpdateStock} currentUser={currentUser} activeUnitId={store.validatedActiveUnitId} sales={store.sales} units={store.units} consignedEvents={store.consignedEvents} setConsignedEvents={store.setConsignedEvents} persist={store.persist} drinksEnabled={isDrinksEnabled} />;
  ...
  return (
    <AuthContext.Provider value={{ currentUser }}>
      ...
      <Sidebar 
        ...
        drinksEnabled={store.units.find(u => u.id === store.validatedActiveUnitId)?.drinksEnabled}
      />
