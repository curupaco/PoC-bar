...
const ShiftControl: React.FC<ShiftControlProps> = ({ shifts = [], onUpdateShifts, currentUser, sales = [] }) => {
  ...
  const handleConfirmClose = () => {
    ...
    const actualCounted = parseVal(actualCountedInput);
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8 pb-20 relative">
      {showConferral && activeShift && (
        <div className="fixed inset-0 z-[100] bg-slate-950 flex items-center justify-center p-0 lg:p-12 animate-in fade-in zoom-in-95 duration-300">
           <div className="bg-white dark:bg-slate-900 w-full h-full max-w-4xl lg:rounded-[60px] shadow-2xl overflow-hidden border border-slate-200 dark:border-slate-800 flex flex-col">
              <div className="p-8 lg:p-12 bg-red-600 text-white shrink-0">
                 <h2 className="text-4xl lg:text-6xl font-black uppercase tracking-tighter italic">Conferência de Caixa</h2>
                 <p className="text-xs lg:text-sm font-black uppercase tracking-widest mt-2 opacity-80">Validação Física de Valores na Gaveta</p>
              </div>
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                    <div className="space-y-6">
                       ...
                       <div className="space-y-4 font-mono text-lg">
                          ...
                          <div className="flex justify-between border-b border-slate-100 dark:border-slate-800 pb-3">
                             <span className="text-slate-500 uppercase">Entradas Dinheiro</span>
                    ...
                    <div className="bg-slate-50 dark:bg-slate-800/50 p-10 rounded-[40px] border border-slate-100 dark:border-slate-800 space-y-6 flex flex-col justify-center">
                       ...
                       <div className="relative">
                          ...
                          <input 
                             ...
                             onChange={e => handleInputChange(e.target.value, setActualCountedInput)} 
                       ...
                       {actualCountedInput && (
                          <div className={`text-center p-4 rounded-2xl font-black uppercase text-xs animate-in zoom-in-95 ${parseVal(actualCountedInput) === expectedCashInDrawer ? 'bg-emerald-500 text-white' : 'bg-amber-500 text-white'}`}>
                             {parseVal(actualCountedInput) === expectedCashInDrawer ? 'CAIXA CONDIZENTE' : `DIFERENÇA: ${formatCurrency(parseVal(actualCountedInput) - expectedCashInDrawer)}`}
                          </div>
                       )}
                    </div>
                 </div>
      ...
      {activeShift ? (
        <div className="bg-slate-900 text-white p-10 rounded-[40px] shadow-2xl relative overflow-hidden animate-in fade-in zoom-in-95 duration-300">
           <div className="relative z-10 space-y-8">
              ...
              <div className="pt-4">
                <button onClick={() => setShowConferral(true)} className="w-full bg-red-600 hover:bg-red-700 py-6 rounded-2xl font-black uppercase text-xs tracking-widest transition-all shadow-xl shadow-red-900/40">FECHAR TURNO E CONFERIR</button>
              </div>
           </div>
        </div>
      ) : (
