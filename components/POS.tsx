    ...
    <div className="flex flex-col lg:flex-row gap-6 relative h-full">
      ...
      {deleteConfirmId && (
        <div className="fixed inset-0 z-[400] flex items-center justify-center p-4">
          ...
          <div className="bg-white dark:bg-slate-900 w-full max-sm:rounded-t-[32px] sm:rounded-[32px] p-10 shadow-2xl relative z-[410] border border-slate-200 dark:border-slate-800 animate-in zoom-in-95 max-sm:absolute max-sm:bottom-0 text-center">
             ...
             <h3 className="text-2xl font-black text-slate-800 dark:text-white uppercase mb-4 tracking-tighter leading-none">Excluir Mesa?</h3>
             <p className="text-sm text-slate-500 dark:text-slate-400 font-medium mb-10 leading-relaxed">
               A mesa "<span className="font-black text-slate-800 dark:text-white uppercase">{deleteConfirmId.name}</span>" possui consumo pendente. Tem certeza que deseja apagar tudo?
             </p>
             <div className="flex flex-col gap-3">
                ...
                }} className="w-full bg-red-600 text-white py-5 rounded-2xl font-black uppercase text-xs tracking-widest active:scale-95 transition-all shadow-lg shadow-red-500/20">Confirmar Exclusão</button>
                <button onClick={() => setDeleteConfirmId(null)} className="w-full py-5 rounded-2xl font-black uppercase text-xs tracking-widest text-slate-400">Cancelar</button>
             </div>
          </div>
        </div>
      )}
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="space-y-1">
                          ...
                          <div className="flex gap-2">
                            ...
                            <button onClick={handleAddPayment} className="bg-black text-white px-6 rounded-2xl font-black active:scale-95 transition-all shadow-lg text-xl">+</button>
                          </div>
                        </div>
                      </div>
      ...
      {(weightModalProduct || editingWeightIndex !== null) && (
        <div className="fixed inset-0 z-[500] flex items-center justify-center p-4 bg-black/90 backdrop-blur-md animate-in fade-in">
          <div className="bg-white dark:bg-slate-900 w-full max-sm:rounded-[40px] sm:max-w-sm sm:rounded-[40px] p-10 shadow-2xl text-center border border-slate-200 dark:border-slate-800">
            ...
            <div className="relative">
               <input autoFocus type="number" inputMode="numeric" value={inputGrams} onChange={e => setInputGrams(e.target.value)} onKeyDown={e => e.key === 'Enter' && (() => {
               ...
               })()} className="w-full text-5xl font-black p-8 text-center rounded-3xl bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-white border-4 border-red-500 outline-none shadow-inner" placeholder="0" />
            </div>
