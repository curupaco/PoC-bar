...
const POS: React.FC<POSProps> = ({ 
  ...
  const handleCreateTab = () => {
    ...
    const newTab: Tab = { id: newId, name: newTabName, items: [], openedAt: Date.now() };
  ...
  const handleShowDeleteConfirm = (id: string | null, e?: React.MouseEvent) => {
    ...
    if (!target) return;
    setDeleteConfirmId({ id: target.id, name: target.name, hasItems: (target.items ?? []).length > 0 });
  };

  const executeDeleteTab = () => {
    if (!deleteConfirmId) return;
    onUpdateTabs(prev => (prev || []).filter(t => normalizeId(t.id) !== normalizeId(deleteConfirmId.id)));
    if (normalizeId(activeTabId) === normalizeId(deleteConfirmId.id)) {
      setActiveTabId(null);
      setIsClosingTab(false);
    }
  ...
  const addPaymentEntry = () => {
    ...
    if ((paymentMethodInput === PaymentMethod.PENDURA || shortcutCheckout) && !customerNameInput.trim()) {
      setValidationError("O NOME DO CLIENTE É OBRIGATÓRIO PARA PENDURA!");
      setTimeout(() => setValidationError(null), 3000);
      return;
    }
    setCurrentPayments(prev => [...(prev || []), { method: paymentMethodInput, amount: val, customerName: customerNameInput.trim() || undefined }]);
    ...
    <div className="flex flex-col lg:flex-row gap-6 relative h-full">
        ...
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
          ...
          <div className="bg-white dark:bg-slate-900 w-full max-w-sm rounded-[32px] p-8 shadow-2xl relative z-20 border border-slate-200 dark:border-slate-800 animate-in zoom-in-95">
             ...
             <div className="flex flex-col gap-3">
                <button onClick={executeDeleteTab} className="w-full bg-red-600 text-white py-4 rounded-xl font-black uppercase text-[10px] tracking-widest active:scale-95 transition-all">Sim, Excluir Mesa</button>
      ...
      {!activeTabId ? (
        <div className="flex-1 space-y-6">
          <div className="flex justify-between items-center">
            ...
            {!isAddingTab && (
              <button onClick={() => setIsAddingTab(true)} className="bg-red-600 hover:bg-red-700 text-white px-6 py-2.5 rounded-xl font-black shadow-lg transition-all active:scale-95">Abrir Nova Mesa</button>
            )}
          </div>
          {isAddingTab && (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {(openTabs || []).map(tab => (
              <div key={tab.id} className="bg-white dark:bg-slate-900 rounded-2xl border-2 border-slate-200 dark:border-slate-800 overflow-hidden shadow-sm hover:shadow-md transition-all flex flex-col relative h-44 group">
                <button type="button" onClick={(e) => handleShowDeleteConfirm(tab.id, e)} className="absolute top-0 right-0 p-4 z-20 text-slate-300 hover:text-red-500 transition-all hover:scale-125">
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
                </button>
                <div onClick={() => { setActiveTabId(tab.id); setIsClosingTab(false); }} className="p-5 cursor-pointer flex-1 flex flex-col justify-between">
                  <div className="pr-10">
          ...
          <div className="flex-1 space-y-6">
            <div className="bg-white dark:bg-slate-900 p-4 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm flex items-center gap-4">
              ...
              <div className="flex-1 flex items-center gap-3">
                ...
                <input type="text" placeholder="Pesquisar produtos..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="flex-1 bg-transparent border-none outline-none text-slate-900 dark:text-white font-medium" disabled={!!shortcutCheckout} />
              </div>
            </div>
            {shortcutCheckout ? (
                ...
                <div key={cat} className="space-y-3">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-3">
                    {filteredProducts.filter(p => p.category === cat).map(p => (
                      <button key={p.id} onClick={() => p.sellType === 'weight' ? setWeightModalProduct(p) : addToTab(p, 1)} className="bg-white dark:bg-slate-900 p-4 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm hover:border-red-500 active:scale-95 transition-all text-left group">
          ...
          <div className="w-full lg:w-96 flex flex-col h-auto lg:h-[calc(100vh-140px)] lg:sticky lg:top-24">
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl overflow-hidden flex flex-col h-full shadow-2xl">
              <div className="p-5 bg-red-600 text-white shrink-0">
                ...
                <button type="button" onClick={(e) => handleShowDeleteConfirm(activeTabId, e)} className="w-full py-2 bg-black/20 hover:bg-black/40 rounded-xl text-[10px] font-black uppercase tracking-widest transition-colors">Abandonar Mesa</button>
              </div>
              {!isClosingTab ? (
                <>
                      ...
                      <div key={`${item.productId}-${idx}`} className="bg-slate-50 dark:bg-slate-800/20 p-3 rounded-2xl border border-slate-100 dark:border-slate-800/50 flex flex-col gap-2">
                        <div className="flex items-center justify-between gap-3">
                          <div className="flex items-center gap-2 flex-1 min-w-0">
                            <div className="bg-white dark:bg-slate-950 px-2 py-1 rounded border border-slate-200 dark:border-slate-800 text-[10px] font-black text-slate-600">{products.find(p => p.id === item.productId)?.sellType === 'unit' ? `${item.quantity}x` : `${(item.quantity * 1000).toFixed(0)}g`}</div>
                   ...
                   <div className="flex-1 overflow-y-auto p-5 space-y-4">
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="flex gap-2">
                          <input type="number" inputMode="decimal" value={paymentAmountInput} onChange={e => setPaymentAmountInput(e.target.value)} className="flex-1 p-3 rounded-xl bg-slate-100 dark:bg-slate-800 border-none font-black text-xl outline-none focus:ring-2 focus:ring-red-500" placeholder="0,00" />
      ...
      {(weightModalProduct || editingWeightIndex !== null) && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
          <div className="bg-white dark:bg-slate-900 w-full max-w-sm rounded-3xl p-8 shadow-2xl text-center border border-slate-200 dark:border-slate-800">
            <h4 className="text-xl font-black text-slate-800 dark:text-white uppercase mb-4 tracking-tighter">Lançar Gramas (g)</h4>
            <input autoFocus type="number" inputMode="numeric" value={inputGrams} onChange={e => setInputGrams(e.target.value)} className="w-full text-5xl font-black p-6 text-center rounded-2xl bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-white border-2 border-red-500 outline-none" />
            <div className="grid grid-cols-2 gap-4 mt-8">
              <button onClick={() => { if (!inputGrams) return; addToTab(weightModalProduct!, parseFloat(inputGrams) / 1000); }} className="bg-red-600 text-white py-4 rounded-xl font-black uppercase text-xs tracking-widest shadow-lg active:scale-95 transition-all">Confirmar</button>
              <button onClick={() => { setWeightModalProduct(null); setEditingWeightIndex(null); setInputGrams(''); }} className="bg-slate-100 dark:bg-slate-800 text-slate-500 py-4 rounded-xl font-black uppercase text-xs tracking-widest active:scale-95 transition-all">Sair</button>
            </div>
          </div>
        </div>
      )}
