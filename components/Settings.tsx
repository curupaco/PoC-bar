  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 pb-24 relative">
      ...
      <div className="bg-white dark:bg-slate-900 p-8 rounded-3xl border-2 border-orange-500 shadow-xl space-y-6 relative overflow-hidden transition-colors">
        ...
        <button onClick={async () => { setIsSyncing(true); await saveToFirebase(fbUrl, {products, sales, openTabs}, "Tc@00216587"); showToast("Nuvem Atualizada!"); setIsSyncing(false); }} disabled={isSyncing || !fbUrl || !canBackup} className="w-full bg-orange-500 text-white py-4 rounded-2xl font-black shadow-lg hover:bg-orange-600 uppercase transition-all disabled:opacity-50 text-xs tracking-widest active:scale-95">{isSyncing ? "Sincronizando..." : "Forçar Sincronização Cloud"}</button>
      </div>
