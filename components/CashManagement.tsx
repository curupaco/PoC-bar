...
const CashManagement: React.FC<CashManagementProps> = ({ shifts, onUpdateShifts, sales, currentUser, onViewChange }) => {
  ...
  const isDrawerOverflowing = currentDrawerBalance > 1000;
  ...
  const handleKeypadPress = (key: string) => {
    ...
    if (key === 'back') { setKeypadValue(prev => prev.slice(0, -1)); return; }
    if (key === '00') {
        setKeypadValue(prev => {
            if (!prev) return '';
    ...
    <div className="max-w-7xl mx-auto space-y-10 pb-32 animate-in fade-in relative">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-6">
            <div className={`relative p-8 rounded-[40px] border-2 transition-all duration-500 group overflow-hidden ${isDrawerOverflowing ? 'bg-orange-50 border-orange-300 dark:bg-orange-900/10 dark:border-orange-500/30' : 'bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800'}`}>
               <div className="flex justify-between items-start relative z-10">
                  <div className="flex items-center gap-4">
                     <div className={`w-14 h-14 rounded-2xl flex items-center justify-center text-2xl shadow-sm ${isDrawerOverflowing ? 'bg-orange-100 text-orange-600' : 'bg-slate-100 dark:bg-slate-800 text-slate-400'}`}>💵</div>
                     <div>
               ...
               <div className="mt-8 flex gap-4 relative z-10">
                  <button onClick={() => { setOperation('SANGRIA'); setKeypadValue(''); }} className="flex-1 bg-red-600 hover:bg-red-700 text-white py-4 rounded-2xl font-black uppercase text-xs tracking-widest shadow-lg shadow-red-600/20 active:scale-95 transition-all flex items-center justify-center gap-2">
            ...
            {/* Cofre Ocupando largura total se antes tinha reserva */}
            <div className="bg-slate-900 text-white p-8 rounded-[40px] shadow-2xl relative overflow-hidden group">
               <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
                  <div>
                    ...
                    <p className="text-4xl font-black tracking-tighter">{formatCurrency(activeShift.cashPrimary)}</p>
                  </div>
                  <button onClick={() => { setOperation('SUPPLY'); setKeypadValue(''); }} className="md:w-auto bg-emerald-600 hover:bg-emerald-500 text-white py-4 px-10 rounded-2xl font-black uppercase text-xs tracking-widest shadow-lg active:scale-95 transition-all flex items-center justify-center gap-2">
                       ...
                       <div className="flex-1 min-w-0">
                          <div className="flex justify-between items-center mb-1">
                             <span className={`text-[10px] font-black uppercase tracking-wide ${t.from === 'Change' ? 'text-red-600' : 'text-emerald-600'}`}>
                                {t.from === 'Change' ? 'SANGRIA' : 'SUPRIMENTO'}
                             </span>
         ...
         <div className="fixed inset-0 z-[500] bg-slate-950/90 backdrop-blur-md flex items-end sm:items-center justify-center sm:p-4 animate-in fade-in duration-200">
            <div className="bg-white dark:bg-slate-900 w-full sm:max-w-sm rounded-t-[40px] sm:rounded-[40px] p-6 sm:p-8 shadow-2xl animate-in slide-in-from-bottom-10 zoom-in-95 duration-300 border border-slate-200 dark:border-slate-800">
               <div className="flex justify-between items-center mb-8">
                  <div>
                     <h3 className={`text-xl font-black uppercase tracking-tighter italic ${operation === 'SANGRIA' ? 'text-red-600' : 'text-emerald-500'}`}>{getOperationLabel(operation)}</h3>
               ...
               <div className="bg-slate-50 dark:bg-slate-950 rounded-3xl p-6 mb-6 text-right border-2 border-slate-100 dark:border-slate-800 shadow-inner relative overflow-hidden">
                  ...
                  <p className={`text-5xl font-black tracking-tighter ${keypadValue ? 'text-slate-800 dark:text-white' : 'text-slate-300'}`}>{displayValue}</p>
               </div>
               <div className="grid grid-cols-3 gap-3 mb-6">
                  {[1, 2, 3, 4, 5, 6, 7, 8, 9].map(num => (
                     <button key={num} onClick={() => handleKeypadPress(num.toString())} className="h-16 rounded-2xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 shadow-sm text-2xl font-black text-slate-700 dark:text-white active:scale-95 active:bg-slate-100 dark:active:bg-slate-700 transition-all">{num}</button>
                  ))}
               ...
               <button onClick={executeTransaction} disabled={!keypadValue || parseInt(keypadValue) === 0} className="w-full bg-slate-900 dark:bg-white text-white dark:text-slate-900 py-6 rounded-3xl font-black uppercase text-sm tracking-[0.2em] shadow-xl active:scale-95 transition-all disabled:opacity-50 disabled:scale-100">Confirmar Operação</button>
            </div>
         </div>
      )}
    </div>
