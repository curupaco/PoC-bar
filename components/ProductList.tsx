...
const ProductList: React.FC<ProductListProps> = ({ products = [], onAdd, onDelete, onUpdate, currentUser }) => {
  ...
  const categoriesList = useMemo(() => Array.from(new Set([
  ...
  const groupedProducts = useMemo(() => {
    ...
    products.forEach(p => {
      const cat = p.category.toUpperCase() || 'GERAL';
  ...
  const handleRenameCategoryBatch = () => {
    ...
    const catUpper = catToRename.toUpperCase();
    const newUpper = newCatName.toUpperCase();
    
    products.forEach(p => {
      if (p.category.toUpperCase() === catUpper) {
  ...
  return (
    <div className="space-y-8 relative">
      <div className="flex flex-col sm:flex-row justify-between items-center bg-white dark:bg-slate-900 p-6 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm gap-4">
        <div>
           <h2 className="text-xl font-black text-slate-800 dark:text-slate-100 uppercase tracking-tighter text-center sm:text-left">Gestão de Cardápio</h2>
           <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1 text-center sm:text-left">{products.length} Itens Cadastrados</p>
        </div>
        <div className="flex gap-2">
           {canEdit && (
             <button onClick={() => setIsRenamingCategory(true)} className="bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 px-4 py-3 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-slate-200 transition-all">Editar Categorias</button>
           )}
           {canEdit && (
             <button onClick={() => { resetForm(); setShowModal(true); }} className="bg-red-600 hover:bg-red-700 text-white px-6 py-3 rounded-2xl font-black shadow-lg shadow-red-500/20 transition-all active:scale-95 flex items-center gap-2 uppercase text-[10px] tracking-widest">Novo Item</button>
           )}
        </div>
      </div>
