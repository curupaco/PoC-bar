...
const ProductList: React.FC<ProductListProps> = ({ 
  ...
  const addOption = () => {
    ...
    setModOptions([...modOptions, { name: newOptName.toUpperCase().trim(), price: parseCurrencyValue(newOptPrice) }]);
  ...
  return (
    <div className="space-y-6">
      <div className="flex bg-white dark:bg-slate-900 p-1.5 rounded-[24px] border border-slate-200 dark:border-slate-800 shadow-sm w-fit overflow-x-auto no-scrollbar">
         ...
         <button onClick={() => setActiveTab('CATEGORIES')} className={`px-6 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap ${activeTab === 'CATEGORIES' ? 'bg-red-600 text-white shadow-lg' : 'text-slate-500'}`}>Vínculos Automáticos</button>
      </div>
      ...
      {activeTab === 'ITEMS' && (
        <div className="space-y-6 animate-in fade-in duration-300">
          <div className="bg-white dark:bg-slate-900 p-6 rounded-[32px] border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col md:flex-row justify-between items-center gap-4">
            <div>
              <h2 className="text-xl font-black text-slate-800 dark:text-white uppercase italic">Cardápio do Bar</h2>
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{products.length} itens cadastrados</p>
            </div>
      ...
      {activeTab === 'GROUPS' && (
        <div className="space-y-6 animate-in fade-in duration-300">
          <div className="bg-white dark:bg-slate-900 p-6 rounded-[32px] border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col md:flex-row justify-between items-center gap-4">
            <div>
              <h2 className="text-xl font-black text-slate-800 dark:text-white uppercase italic">Menus de Opções / Serviços</h2>
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Crie variações de sabor, serviços de mesa ou opcionais</p>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
             {modifierGroups.map(group => (
               <div key={group.id} className="bg-white dark:bg-slate-900 p-8 rounded-[40px] border border-slate-200 dark:border-slate-800 shadow-sm relative overflow-hidden flex flex-col h-full">
                  <div className="flex justify-between items-start mb-6">
                     <div>
                        <h4 className="font-black uppercase text-lg text-slate-800 dark:text-white">{group.name}</h4>
                        <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Opções: {group.options.length}</p>
                     </div>
                     <div className="flex gap-1">
                  ...
                  <div className="space-y-2 flex-1">
                     {group.options.slice(0, 5).map((opt, i) => (
                       <div key={i} className="flex justify-between text-[11px] font-bold py-1 border-b border-slate-50 dark:border-slate-800">
                          <span className="text-slate-500 uppercase">{opt.name}</span>
                          <span className="text-emerald-500">+{formatCurrency(opt.price)}</span>
                       </div>
                     ))}
                     {group.options.length > 5 && <p className="text-[10px] text-slate-400 italic text-center pt-2">+ {group.options.length - 5} outras opções</p>}
                  </div>
               </div>
             ))}
             {modifierGroups.length === 0 && (
               <div className="col-span-full py-20 text-center text-slate-400 font-black uppercase tracking-widest text-xs border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-[40px]">
                 Nenhum menu de opções cadastrado.
               </div>
             )}
          </div>
        </div>
      ...
      {activeTab === 'CATEGORIES' && (
        <div className="space-y-6 animate-in fade-in duration-300 max-w-3xl mx-auto">
           <div className="bg-white dark:bg-slate-900 p-8 rounded-[40px] border border-slate-200 dark:border-slate-800 shadow-sm text-center">
              ...
              <p className="text-xs text-slate-500 mt-2">Associe categorias inteiras a menus de opções para agilizar o lançamento.</p>
           </div>
                ...
                <div key={cat} className="bg-white dark:bg-slate-900 p-6 rounded-[28px] border border-slate-200 dark:border-slate-800 shadow-sm flex items-center justify-between">
                   ...
                   <div className="w-64">
                      <select 
                        ...
                        className="w-full p-4 rounded-2xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 font-bold uppercase text-[10px] outline-none"
                      >
              ...
              <div className="space-y-4">
                 ...
                 <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="space-y-1">
                        ...
                        <input type="text" value={category} onChange={e => setCategory(e.target.value)} list="existing-cats" placeholder="EX: BEBIDAS" className="w-full p-5 rounded-2xl bg-slate-50 dark:bg-slate-950 font-black uppercase text-xs outline-none border border-transparent focus:border-red-500 shadow-inner" />
                 ...
                 <div className="space-y-1">
                    <label className="text-[10px] font-black text-slate-400 uppercase ml-2">Menu de Opções Individual</label>
                    <select value={modGroupId} onChange={e => setModGroupId(e.target.value)} className="w-full p-5 rounded-2xl bg-slate-50 dark:bg-slate-950 font-black uppercase text-xs outline-none shadow-inner">
                      <option value="">Nenhum (Usar padrão da categoria)</option>
                      {modifierGroups.map(g => <option key={g.id} value={g.id}>{g.name}</option>)}
                    </select>
                 </div>
        ...
        <div className="fixed inset-0 z-[500] flex items-center justify-center p-4 bg-slate-950/90 backdrop-blur-md">
           <div className="bg-white dark:bg-slate-900 w-full max-w-2xl rounded-[40px] p-10 border border-slate-200 dark:border-slate-800 shadow-2xl animate-in zoom-in-95 flex flex-col max-h-[90vh]">
              ...
              <div className="space-y-6 flex-1 overflow-y-auto no-scrollbar">
                 <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1">
                        <label className="text-[10px] font-black text-slate-400 uppercase ml-2">Identificação do Menu</label>
                        <input autoFocus type="text" value={modGroupName} onChange={e => setModGroupName(e.target.value)} placeholder="EX: TAXA DE SERVIÇO" className="w-full p-4 rounded-2xl bg-slate-50 dark:bg-slate-950 font-black uppercase text-sm outline-none shadow-inner" />
                    </div>
                    <div className="space-y-1">
                        <label className="text-[10px] font-black text-slate-400 uppercase ml-2">Agrupador (Opcional)</label>
                        <input type="text" value={modGroupCategory} onChange={e => setModGroupCategory(e.target.value)} placeholder="EX: SERVIÇOS" className="w-full p-4 rounded-2xl bg-slate-50 dark:bg-slate-950 font-black uppercase text-sm outline-none shadow-inner" />
                    </div>
                 </div>

                 <div className="bg-slate-50 dark:bg-slate-950 p-6 rounded-[32px] space-y-4 shadow-inner">
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">Adicionar Novas Opções</p>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                       <input type="text" value={newOptName} onChange={e => setNewOptName(e.target.value)} placeholder="NOME DA OPÇÃO" className="md:col-span-2 p-4 rounded-xl bg-white dark:bg-slate-900 font-bold uppercase text-xs outline-none border border-slate-100 dark:border-slate-800" />
                       <div className="flex gap-2">
                          <input type="text" value={newOptPrice} onChange={e => setNewOptPrice(sanitizeCurrencyInput(e.target.value))} placeholder="0,00" className="flex-1 p-4 rounded-xl bg-white dark:bg-slate-900 font-bold text-emerald-500 outline-none border border-slate-100 dark:border-slate-800" />
                          <button onClick={addOption} className="bg-black text-white w-12 h-12 rounded-xl font-black text-xl">+</button>
                       </div>
                    </div>
                    <div className="pt-4 space-y-2">
                       {modOptions.map((opt, i) => (
                         <div key={i} className="flex justify-between items-center bg-white dark:bg-slate-900 p-3 rounded-xl border border-slate-100 dark:border-slate-800 animate-in slide-in-from-right-2">
                            <div className="flex flex-col">
                               <span className="text-[10px] font-black uppercase text-slate-800 dark:text-white">{opt.name}</span>
                               <span className="text-[9px] font-bold text-emerald-500">+{formatCurrency(opt.price)}</span>
                            </div>
                            <button onClick={() => removeOption(i)} className="text-red-500 p-2 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg">
                               <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M6 18L18 6M6 6l12 12" /></svg>
                            </button>
                         </div>
                       ))}
                       {modOptions.length === 0 && <p className="text-[10px] text-slate-400 italic text-center py-4">Nenhuma opção adicionada ainda.</p>}
                    </div>
                 </div>
              </div>

              <div className="flex gap-4 pt-8">
                 <button onClick={handleSaveModGroup} className="flex-1 bg-blue-600 text-white py-5 rounded-2xl font-black uppercase shadow-xl active:scale-95 transition-all">Salvar Menu</button>
